...
createApp({
    setup() {
        ...
        const filteredPool = computed(() => {
            ...
            // Filtro invisible: Oculta los que ya están en Blacklist o Cuentas
        ...
        const fetchCurrentIP = async (uid) => {
            ...
            if(!newIp) { try { const r2 = await fetch("https://ipinfo.io/json"); if(r2.ok) { const d2 = await r2.json(); newIp = d2.ip; } } catch(e) {} }
        ...
        const confirmAssignFromPool = (accountUid) => {
            ...
            if (acc && nodeToAssign.value) {
                ...
                acc.ip = ''; acc.previousIp = null; acc.worked = false;
        ...
        const processNodeData = (data) => {
            ...
            data.forEach(nodo => {
                // Filtramos por Wireguard y que tenga ID
                if (!nodo.provider_id) return;
                
                const idCorto = nodo.provider_id.substring(0, 14);
                if(seenIds.has(idCorto)) return; // Evitar duplicados en la misma API
                
                // Evitamos procesar los que ya sabemos que están baneados (optimiza memoria)
                ...
                if (!isBlacklisted && !isUsed) {
                    ...
                    rawPool.push({
                        ...
                        asn_isp: `${nodo.location?.asn || ''} - ${nodo.location?.isp || 'Desconocido'}`.replace(/^- | -$/g, ''),
        ...
        // ==========================================
        // 🚀 CASCADA MYSTERIUM CORREGIDA (FIX CORS)
        // ==========================================
        const fetchAPI = async () => {
            ...
            // Proxies ordenados del más estable al menos estable para saltar el CORS
            const attempts = [
                { name: 'Proxy 1 (AllOrigins Seguro)', url: `https://api.allorigins.win/get?url=${encodeURIComponent(targetUrl)}`, type: 'allorigins' },
                { name: 'Proxy 2 (CORS Proxy)', url: `https://corsproxy.io/?${encodeURIComponent(targetUrl)}`, type: 'direct' },
                { name: 'Directo', url: targetUrl, type: 'direct' },
            ...
            for (let attempt of attempts) {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        // Si es AllOrigins, la respuesta real de Mysterium viene dentro de "contents" en formato texto
            ...
            if (data && Array.isArray(data)) {
                ...
                alert("🚫 BLOQUEO CORS ACTIVO\n\nTu red o navegador bloqueó los 4 proxies de seguridad. \n\nUsa el botón 'Importar JSON' para subir la lista manualmente.");
        ...
        const importPoolJSON = (event) => {
            ...
            reader.onload = (e) => {
                ...
                } catch (err) { alert("Error al leer el archivo JSON."); event.target.value = null; }
            };
